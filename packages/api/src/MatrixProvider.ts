...
export default class MatrixProvider implements IChatProvider {
    ...
    async connect(): Promise<void> {
        ...
        this.client.on("Room.timeline", (event: any, room: any, toStartOfTimeline: boolean) => {
            ...
            const text = isEncrypted ? "⚠️ [Encrypted Message]" : event.getContent().body;
        ...
        if (!room) {
            try {
                ...
                console.error(`MatrixProvider: Failed to join room ${this.roomId}`, error);
            }
        }
    }

    async login(userOrEmail: string, password: string, code?: string): Promise<any> {
        if (!this.client) {
            this.client = sdk.createClient({
                baseUrl: this.host,
            });
    ...
    async getMessages(anonymousMode: boolean, options?: any, isChannelPrivate?: boolean): Promise<any> {
        ...
        const messages = events
            ...
            .reverse()
            .filter((event: any) => event.getType() === "m.room.message" || event.getType() === "m.room.encrypted")
            .map((event: any) => {
                const isEncrypted = event.getType() === "m.room.encrypted";
                const text = isEncrypted ? "⚠️ [Encrypted Message]" : event.getContent().body;
                return {
    ...
    async sendMessage(message: any, threadId?: string): Promise<any> {
        ...
        const response = await this.client.sendEvent(this.roomId, "m.room.message", content);
        return { _id: response.event_id };
    }

    async getOlderMessages(anonymousMode: boolean, options?: any, isChannelPrivate?: boolean): Promise<any> {
        return { messages: [] };
    }

    async getThreadMessages(tmid: string, isChannelPrivate?: boolean): Promise<any> {
        return { messages: [] };
    }
    ...
    async autoLogin(auth: { flow: "PASSWORD" | "OAUTH" | "TOKEN"; credentials: any }): Promise<void> {
    }
