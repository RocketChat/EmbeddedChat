...
const Prune = () => {
  ...
  const roomData = mentionmemberStore((state) => state.roomMembers);
  const getRoomMembers = roomData.map((i) => i.username);
  ...
  const handlePrune = async () => {
    ...
    await RCInstance.cleanHistory(data);
    setMessages(d);
    togglePrune();
  };

  return (
    <Box className={classes.NavBar} p="x16">
