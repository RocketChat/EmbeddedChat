...
**Extract plan metadata for telemetry:**

```bash
PLAN_ID=$(basename "$PLAN_FILE" | sed 's/-PLAN.md//')
PHASE_NUM=$(echo "$PLAN_ID" | cut -d- -f1)
PHASE_NAME=$(basename "$(dirname "$PLAN_FILE")" | sed 's/^[0-9]*-//')
WAVE_NUM=$(grep "^wave:" "$PLAN_FILE" | cut -d: -f2 | tr -d ' ' || echo "1")
```

**Start telemetry tracking:**

```bash
if type telemetry_start >/dev/null 2>&1; then
  # CONCURRENT_COUNT should be passed from orchestrator, default to 1
...
**Record telemetry before completion:**

```bash
# Calculate tokens used (estimate from plan + context)
PLAN_TOKENS=$(telemetry_estimate_tokens "$(cat $PLAN_FILE)" 2>/dev/null || echo "12000")
STATE_TOKENS=$(telemetry_estimate_tokens "$(cat .gsd/STATE.md 2>/dev/null)" 2>/dev/null || echo "3000")
TOTAL_TOKENS=$((PLAN_TOKENS + STATE_TOKENS))

# Determine model used (from config or default)
MODEL_USED=$(cat .gsd/config.json 2>/dev/null | grep -o '"model_profile"[[:space:]]*:[[:space:]]*"[^"]*"' | grep -o '"[^"]*"$' | tr -d '"' || echo "sonnet")
# Map profile to actual model (balanced → sonnet)
case "$MODEL_USED" in
  balanced) MODEL_USED="sonnet" ;;
  quality) MODEL_USED="opus" ;;
  budget) MODEL_USED="haiku" ;;
esac
...
# Record telemetry
if type telemetry_end >/dev/null 2>&1; then
